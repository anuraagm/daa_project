...
if __name__ == "__main__":
    ...
    for it in range(10):
        ...
        while array_size <= 2000:
            ...
            start = monotonic()
            merge_sort(randomized_array, 0, array_size-1)
            end = monotonic()
            elapsed_time = end - start

            output_write_path = './io/merge_sort/'+(str(array_size)+"_output_"+str(it)+".txt")
            with open(output_write_path,'w') as f:
                for elem in randomized_array:
                    f.write(str(elem)+", ")

            print("Time taken to sort ",array_size," elements, using Merge Sort = ", elapsed_time, " seconds")
